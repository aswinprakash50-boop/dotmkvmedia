...
export default function EmployeeHub({ data, selectedEmployeeName }: EmployeeHubProps) {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fadeIn">
      ...
      <div className="glass-panel rounded-2xl p-2.5 sm:p-3 border border-white/5 flex items-center gap-2 overflow-x-auto no-scrollbar">
        <span className="text-[11px] sm:text-xs font-semibold text-gray-400 px-2 sm:px-3 uppercase tracking-wider shrink-0 flex items-center gap-1.5">
          ...
          {employees.map((name) => {
            ...
            return (
              <button
                ...
                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs font-semibold transition-all shrink-0 flex items-center gap-1.5 sm:gap-2 cursor-pointer ${
                  isSelected
      ...
      <div className="rounded-2xl p-4 sm:p-6 bg-gradient-to-r from-gray-900 via-indigo-950/30 to-gray-900 border border-indigo-500/20">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-tr from-indigo-600 to-violet-600 flex items-center justify-center text-white text-lg sm:text-xl font-black shadow-xl shadow-indigo-500/20 border border-indigo-400/30 shrink-0">
              {emp.name.charAt(0)}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl sm:text-2xl font-black text-white">{emp.name}</h2>
                <span className="px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 text-[9px] sm:text-[10px] font-semibold uppercase tracking-wider border border-indigo-500/30">
                  Video Editor
                </span>
              </div>
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            <div className="flex-1 sm:flex-initial px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-gray-900/80 border border-gray-800 text-center">
              <span className="text-[9px] sm:text-[10px] text-gray-400 uppercase font-semibold block">Delivery Rate</span>
            ...
            <div className="flex-1 sm:flex-initial px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-gray-900/80 border border-gray-800 text-center">
              <span className="text-[9px] sm:text-[10px] text-gray-400 uppercase font-semibold block">Days Active</span>
              <span className="text-base sm:text-lg font-black text-indigo-400">{emp.daysWorked} days</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2.5 sm:gap-4">
        <div className="glass-panel rounded-2xl p-3.5 sm:p-4 border border-white/5">
          <div className="flex items-center justify-between text-gray-400 mb-1.5 sm:mb-2">
            <span className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider">Total Output</span>
            <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-400" />
          </div>
          <div className="text-xl sm:text-2xl font-black text-white">{emp.totalHours} <span className="text-xs font-normal text-gray-400">h</span></div>
          <div className="text-[10px] sm:text-[11px] text-gray-400 mt-1 truncate">
        ...
        <div className="glass-panel rounded-2xl p-3.5 sm:p-4 border border-white/5">
          ...
          <div className="text-xl sm:text-2xl font-black text-emerald-400">{emp.distinctProjectCount}</div>
          <div className="text-[10px] sm:text-[11px] text-emerald-300/80 mt-1 truncate">
            {emp.deliveredProjectCount} deliv • {emp.inProgressProjectCount} act
          </div>
        </div>

        <div className="glass-panel rounded-2xl p-3.5 sm:p-4 border border-white/5">
          <div className="flex items-center justify-between text-gray-400 mb-1.5 sm:mb-2">
            <span className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider">Daily Pace</span>
            <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-cyan-400" />
          </div>
          <div className="text-xl sm:text-2xl font-black text-cyan-400">{emp.avgHoursPerDay} <span className="text-xs font-normal text-gray-400">h/d</span></div>
          <div className="text-[10px] sm:text-[11px] text-gray-400 mt-1 truncate">
            Active day average
          </div>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-3.5 sm:p-4 border border-white/5">
          ...
          <div className="text-xl sm:text-2xl font-black text-amber-400">
            {emp.avgTatDays !== null ? `${emp.avgTatDays} d` : '1.3 d'}
          </div>
          <div className="text-[10px] sm:text-[11px] text-gray-400 mt-1 truncate">
            Turnaround time
          </div>
        ...
        <div className="glass-panel rounded-2xl p-3.5 sm:p-4 border border-white/5 col-span-2 md:col-span-4 lg:col-span-1">
          <div className="flex items-center justify-between text-gray-400 mb-1.5 sm:mb-2">
            <span className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider">Client Accounts</span>
            <Building2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-400" />
          </div>
          <div className="text-xl sm:text-2xl font-black text-purple-400">{emp.activeClients.length}</div>
          <div className="text-[10px] sm:text-[11px] text-gray-400 mt-1 truncate">
        ...
        <div className="glass-panel rounded-2xl p-4 sm:p-6 border border-white/5">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div>
              ...
              <p className="text-[11px] sm:text-xs text-gray-400 mt-0.5">Hours logged per day in September</p>
            </div>
            <span className="text-[11px] sm:text-xs font-mono text-gray-400 bg-gray-800 px-2.5 py-1 rounded-lg">
              Max {maxEmpDaily}h
            </span>
          </div>

          <div className="overflow-x-auto no-scrollbar pb-2">
            <div className="h-40 sm:h-44 min-w-[320px] w-full flex items-end gap-2 pt-4 pb-2 border-b border-gray-800">
        ...
        <div className="glass-panel rounded-2xl p-4 sm:p-6 border border-white/5">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div>
              ...
              <p className="text-[11px] sm:text-xs text-gray-400 mt-0.5">Hours spent across different client accounts</p>
            </div>
          </div>

          <div className="space-y-2.5 sm:space-y-3 mt-2 max-h-52 overflow-y-auto pr-1">
            {emp.clientsBreakdown.map((c) => {
              ...
              return (
                <div key={c.client} className="p-2 sm:p-2.5 rounded-xl bg-gray-900/40 border border-gray-800">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="font-bold text-gray-300 truncate max-w-[180px] sm:max-w-[200px]">{c.client}</span>
                    <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <tbody className="divide-y divide-gray-800/60">
              {employeeProjects.map((p) => (
                <tr key={p.id} className="hover:bg-gray-800/30 transition-colors">
                  <td className="py-3 px-3 font-semibold text-gray-200 max-w-xs truncate">
                    {p.projectName}
                  </td>
                  <td className="py-3 px-3 text-gray-400">
                    <span className="px-2 py-0.5 rounded bg-gray-900 border border-gray-800 text-gray-300 font-medium whitespace-nowrap">
                      {p.client}
                    </span>
                  </td>
